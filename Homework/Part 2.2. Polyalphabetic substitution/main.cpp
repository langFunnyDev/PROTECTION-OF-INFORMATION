...
int main() {
    ...
    std::cout << "Hello, the encryption program using polyalphabetic substitution welcomes you." << std::endl
    ...
    if (selectedWorkMode == 1) {
        ...
        for (int i = 0, j = 0; i <= originalText.size(); ++i) {
            for (; j <= encryptionKey.size() - 1; ++j) {
                encryptedText += (char) ((int)originalText[i] + (int)encryptionKey[j]);
                if (j == encryptionKey.size() - 1) j = 0;
                break;
            }
        }
        ...
        for (int i = 0, j = 0; i <= encryptedText.size(); ++i, ++j) {
                originalText += (char) ((int)encryptedText[i] - (int)encryptionKey[j]);
                if (j == encryptionKey.size() - 1) j = 0;
        }
